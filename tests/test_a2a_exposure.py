...
def test_a2a_exposure_wave_fires():
    assert {"ATL-123", "ATL-208"} <= _ids(build_model(FIXTURE))
...
def test_external_reach_names_endpoint_and_capability():
    (f,) = [f for f in _findings(build_model(FIXTURE)) if f.rule_id == "ATL-208"]
...
def test_multiagent_fixture_now_flags_external_reach():
    ...
    assert "ATL-208" in _ids(build_model("examples/multi-agent"))
