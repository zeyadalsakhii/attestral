...
def test_attack_path_fires_on_full_chain():
    assert "ATL-210" in rule_ids(build_model(FIXTURE))
...
def test_no_external_entry_no_path(tmp_path):
    ...
    assert "ATL-210" not in rule_ids(model)
    assert "ATL-203" in rule_ids(model)   # the internal shell+network pair still fires
...
def test_cloud_credential_counts_as_impact(tmp_path):
    ...
    assert "ATL-210" in rule_ids(model)
